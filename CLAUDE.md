...
- **TransferFunctionEstimator**: Wiener deconvolution H(f) = Syx(f)/(Sxx(f)+λ), Bode plot (magnitude+phase), IFFT synthetic step response, works with any flight data (not just stick-snaps)
- **PIDAnalyzer**: Orchestrator with async progress reporting, threads `flightPIDs` through pipeline, transfer function estimation
