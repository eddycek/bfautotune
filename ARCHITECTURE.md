...
**Last Updated:** March 1, 2026 | **Phase 4 Complete, Phase 6 Complete** | **1910 unit tests, 97 files + 16 Playwright E2E tests**
...
| `BayesianOptimizer.ts` | ~280 | 23 | GP-based multi-flight optimization, Expected Improvement acquisition, history-driven gain suggestions |
| `PIDAnalyzer.ts` | ~195 | 22 | PID analysis orchestrator (FF context, flight style, data quality, Bayesian optimization) |
...
| `analysis.types.ts` | `PowerSpectrum`, `NoiseProfile`, `FilterRecommendation`, `StepResponse` (with `ffDominated`), `StepResponseTrace`, `PIDRecommendation`, `AxisStepMetrics`, `CurrentFilterSettings`, `FeedforwardContext`, `BayesianObservation`, `BayesianOptimizationResult` |
...
**1910 unit tests across 97 files + 16 Playwright E2E tests**. See [TESTING.md](./TESTING.md) for complete inventory.

| Area | Files | Tests |
|------|-------|-------|
| Blackbox Parser | 9 | 245 |
| FFT Analysis (+ Data Quality) | 6 | 151 |
| Step Response + Bayesian (+ Real-data) | 6 | 151 |
