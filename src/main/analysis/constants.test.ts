...
describe('PID_STYLE_THRESHOLDS', () => {
  ...
  it('has all required threshold fields for each style', () => {
    const requiredKeys = [
      'overshootIdeal', 'overshootMax', 'settlingMax',
      'ringingMax', 'moderateOvershoot', 'sluggishRise',
    ];
    for (const style of styles) {
  ...
  it('smooth has stricter overshoot thresholds than balanced', () => {
    expect(PID_STYLE_THRESHOLDS.smooth.overshootIdeal).toBeLessThan(PID_STYLE_THRESHOLDS.balanced.overshootIdeal);
    expect(PID_STYLE_THRESHOLDS.smooth.overshootMax).toBeLessThan(PID_STYLE_THRESHOLDS.balanced.overshootMax);
  });

  it('aggressive has more permissive overshoot thresholds than balanced', () => {
    expect(PID_STYLE_THRESHOLDS.aggressive.overshootIdeal).toBeGreaterThan(PID_STYLE_THRESHOLDS.balanced.overshootIdeal);
    expect(PID_STYLE_THRESHOLDS.aggressive.overshootMax).toBeGreaterThan(PID_STYLE_THRESHOLDS.balanced.overshootMax);
  });

  it('smooth allows more settling time, aggressive demands less', () => {
    expect(PID_STYLE_THRESHOLDS.smooth.settlingMax).toBeGreaterThan(PID_STYLE_THRESHOLDS.balanced.settlingMax);
    expect(PID_STYLE_THRESHOLDS.aggressive.settlingMax).toBeLessThan(PID_STYLE_THRESHOLDS.balanced.settlingMax);
  });

  it('sluggish rise threshold scales inversely with aggression', () => {
    expect(PID_STYLE_THRESHOLDS.smooth.sluggishRise).toBeGreaterThan(PID_STYLE_THRESHOLDS.balanced.sluggishRise);
    expect(PID_STYLE_THRESHOLDS.aggressive.sluggishRise).toBeLessThan(PID_STYLE_THRESHOLDS.balanced.sluggishRise);
  });
});
